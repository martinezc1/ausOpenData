#' Pull any Austin Open Data dataset from a Socrata JSON endpoint
#'
#' Downloads data from any Austin Open Data Socrata JSON endpoint and returns
#' the result as a tibble. This function is useful for datasets that are not
#' included in the curated catalog returned by [aus_list_datasets()].
#'
#' Austin Open Data datasets have Socrata JSON endpoints that usually follow
#' this pattern:
#'
#' `https://data.austintexas.gov/resource//<dataset_uid>.json`
#'
#' For example, a dataset with the Socrata UID `"9t4d-g238"` would have the JSON
#' endpoint:
#'
#' `https://data.austintexas.gov/resource/9t4d-g238.json`
#'
#' Users can find a dataset's UID from the Austin Open Data Portal URL, the API
#' documentation page for the dataset, or the output of [aus_list_datasets()].
#'
...
#'
#' @details
#' `aus_any_dataset()` bypasses the package catalog and sends a request directly
#' to the supplied JSON endpoint. Unlike [aus_pull_dataset()], it does not look
...
#' workflows using readable keys or Socrata UIDs, use [aus_pull_dataset()].
#'
#' @examples
...
#'
#' @export
aus_any_dataset <- function(json_link,
                            limit = 10000,
                            timeout_sec = 30,
                            clean_names = TRUE,
                            coerce_types = TRUE) {
