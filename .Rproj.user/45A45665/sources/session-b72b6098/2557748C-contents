#' Pull a Austin Open Data dataset
#'
#' Downloads a dataset from the Austin Open Data Socrata API using either a
#' human-readable catalog `key` or the official Socrata dataset `uid` returned by
#' [aus_list_datasets()].
#'
#' When a catalog `key` is supplied, `aus_pull_dataset()` first retrieves the
#' live Austin Open Data catalog to look up the corresponding Socrata `uid`,
...
#'   [aus_list_datasets()]. For example, a key may look like
...
#'   Austin Open Data Portal or by pulling a small sample with `limit`.
#' @param where Optional raw SoQL `WHERE` clause for advanced filtering. SoQL is
#'   the Socrata Query Language used by Austin Open Data. If `date`, `from`,
...
#'   inconsistent.
#'
#' @return A tibble containing rows from the requested Austin Open Data
#'   dataset.
#'
#' @details
#' `aus_pull_dataset()` is designed for common catalog-based workflows. For
...
#'   aus_pull_dataset("example_dataset_name", limit = 3)
#'
#'   # Pull by Socrata UID
#'   aus_pull_dataset("9t4d-g238", limit = 3)
#'
#'   # Filter to one value
#'   aus_pull_dataset(
#'     "9t4d-g238",
#'     limit = 3,
#'     filters = list(animal_type = "Other")
#'   )
#'
...
#'     "abcd-1234",
#'     limit = 10,
#'     filters = list(animal_type = c("Other", "Bird"))
#'   )
#'
#'   # Date filtering
#'   aus_pull_dataset(
...
#'   aus_pull_dataset(
#'     "abcd-1234",
#'     where = "animal_type = 'Other' AND outcome_type = 'Partner'",
...
#'
#' @export
aus_pull_dataset <- function(dataset,
                             limit = 10000,
                             filters = list(),
                             date = NULL,
                             from = NULL,
  ...
  if (nrow(row) == 0) {
    stop(
      paste0(
        ...
        "Use `aus_list_datasets()` to see available keys and UIDs."
      ),
      call. = FALSE
    )
  }
  ...
  date_where <- .aus_build_date_where(
